...
**Update — the rehearsing reconnoiterer (Task #21, 2026-05-11, shipped)**: implemented inside `LlmReconnoiterer.recon()` + a new adapter-internal `RehearsalWalker` (`src/adapters/recon/rehearsal.ts`, depends only on `IPageSession` + a `reconverge` callback — no port change). `recon()` now does `plan → resolve targets → RehearsalWalker.walk() → reset → return`: the walk runs **off-camera before `beginRecording()`**, walking the draft `Performance` *instantly* (scrolls at `durationMs: 1`, dwells skipped — the plan's animation params are kept untouched for the on-camera render), actually executing each click/type/key/back, **re-resolving each click/type target at the actual scroll position the step runs in** (the eager resolve at scrollY 0 may have missed or mis-matched it; the returned viewport-relative bbox is converted to page-absolute, `y += scrollY`), then observing: divergence if the step threw / landed on `about:blank` / the page didn't change at all after a click/type (a dead element — the §0034 failure this exists to catch) / an `expectAfter` is unsatisfied *and* the page didn't visibly change. Not diverged → rewrite the step's `expectAfter` to the *observed* post-state (`urlContains` = the new path, `visibleText` = stable visible anchors). Diverged → call the `reconverge` callback (capped at `reconReconvergeMax`, default 1; deadline `reconRehearsalBudgetMs`, default 45 000 — lowered from 90 000 / 2 after robustness sweep #1: a 90 s walk + 2 vision-call reconverges blew the whole ~60 s wall-clock budget on heavy pages and still truncated) — "step N intended X, actually Y; here's the current page; give me the rest from here" — splice its steps in and continue; cap/deadline hit → truncate + graceful tail (`scroll` + `dwell` + `done`), set `truncated`/`timedOut`. After the walk: `session.goto(input.url)` + `waitForVisualStability` to reset for the on-camera run. The returned `Performance` gains an optional `rehearsal: {walkedSteps, divergences, reconverges, truncated, timedOut}` field (the operator canary — analogous to `replanCount`); `RunMetrics.rehearsal` surfaces it. Gated by `config.reconRehearse` (default `true`; `RECON_REHEARSE=false` → no walk, behaviour as before). Also in this task: the recon prompt grew a **PRIORITY #1 — accomplish the goal** mandate (extract every requested action; each must be a step; re-read the intent and re-check the steps array before emitting JSON) and **scroll-to-target discipline** (think in viewport-heights; no overshoot; never scroll-past-then-back; "slow scroll" = a few moderate scrolls + brief dwells); the `reconverge` prompt was changed from "do NOT repeat the failed step" to "the failed step's GOAL still matters — reach the same outcome a *different* way (different element / scroll first / dwell for late content), don't blindly re-issue the exact same action"; and `resolveSteps` no longer eagerly *drops* unresolvable click/type steps — it keeps them with an `UNRESOLVED_SENTINEL` target so the walk gets a chance to re-resolve them at the right scroll position (still unresolvable there → divergence → reconverge; a final defensive filter drops any sentinel that somehow survives). **Validation**: re-ran the canonical scenario — verdict went `probably_synthetic` → **`LOOKS_HUMAN`** (all five judge dimensions `pass`); `intentSatisfaction` `partial` → **`complete`** (the 简体中文 click is verified off-camera, lands first try); trimmed video 6.9 s → 10.6 s (on target); `rehearsal: {walkedSteps: 8, divergences: 0, reconverges: 0, truncated: false}`; `replanCount` 0; wall-clock 53 s → 72 s (the walk adds ~20 s, all off-camera). Known remaining gap: multi-screenful "scroll down then click X" where X is *above* the scroll position (Wikipedia "Cat → Felidae" — the taxobox link scrolls away; a long page has several same-text "Felidae" links `resolveTarget` can't disambiguate) → the walk can't re-resolve it, reconverges twice, truncates. Tracked in the findings doc; the next recon-plan-quality lever.

**Open risk / next**:
- A recon-prompt rule for "scroll *past* then click an *earlier* target" — scroll the target back into view (or click it before scrolling away). And `resolveTarget` disambiguation when several elements share the target text.
- The scroll-rendering question is resolved for the canonical scenario (the recon now plans a few moderate paced scrolls, not one fling, and the video is genuinely 25 fps / smooth — the judge prompt's rule F2 also notes it samples ~1 fps so multi-second smooth scrolls aren't "teleports"); revisit only if a future judge run flags it again.
...
## 0035 · Off-camera blocker dismisser (Task #20 — BlockerPrelude reborn)

**Date**: 2026-05-12

**Context**: §0034 Task 8 deleted `BlockerPrelude` (it was coupled to the removed `IFastDecider`); the interim stopgap — "the recon prompt asks the LLM to plan an accept-cookies click as the first `Performance` step" — failed in robustness sweep #1 (`docs/findings/2026-05-11-robustness-sweep-1.md`): on `theguardian.com` the recon planned a click on "the first headline" but never a dismiss-consent step (the consent iframe loads after the recon screenshot, or the LLM just doesn't reliably plan it), so every click landed on the overlay, didn't navigate, the rehearsal walk diverged → reconverged → re-failed → truncated → `intentSatisfaction: unknown`. A consent/dismissable overlay covering the page is extremely common on the open web; dismissing it off-camera, before the `Performance` plays, is free and lets the recon plan against the real page. Serves goals.md #2 (no visible stalls — blocker-hunting isn't in the deliverable) and #3.

**Choice**: new port `IBlockerDismisser` (`src/ports/blocker-dismisser.ts`) + adapter `LlmBlockerDismisser` (`src/adapters/blocker/llm-blocker-dismisser.ts`, its own `gpt-4o-mini` OpenRouter client — `config.llmBlockerModelResolved`, default `LLM_MODEL`), owned by `LlmReconnoiterer` (optional ctor field `blockerDismisser`; `IReconnoiterer.recon(input, session)` signature unchanged). It runs a **probe → detect → click → re-probe loop**: probe `session.pageDiagnostic().blockerSignals` — empty ⇒ return immediately, zero LLM calls (the common case); else, up to `blockerDismissMaxRounds` (3) iterations within `blockerDismissMaxMs` (10 000): screenshot + `observeAll()` → one vision LLM call (`{blocker, dismissTargetDescription?, rationale?}`, strict JSON) → if `blocker:true` resolve+click the dismiss control (coord-click in viewport, else `clickSelector`) → `waitForVisualStability` → re-probe `blockerSignals` (empty ⇒ done, else loop). Best-effort: any throw / unparseable response / unresolvable target / failed click ⇒ stop with `stillBlocked: true`; a `dismiss()` that throws is caught by `recon()` ⇒ proceeds anyway. `recon()` calls it at the top (before `observeAll`, so the plan is built against a clean page) **and** inside the rehearsal walk's page-reset (after `goto(input.url)` — the walk reloaded the page, the overlay can be back, the on-camera Director must see a clean page too). The outcome rides on `Performance.blockerDismissal: {rounds, dismissed[], stillBlocked}` (like `rehearsal`) → `RunMetrics.blockerDismissal`. Runs entirely before `beginRecording()` ⇒ **not** in the action log. Gated by `config.blockerDismiss` (default on; `BLOCKER_DISMISS=false` ⇒ no dismisser is constructed, behaviour as before). Scope: cookie/consent banners + X-to-close modals (newsletter, app-install) only — **not** region/age gates (a real choice) or paywalls/login-walls (undismissable — left for the recon to plan around).

**Consequences**: zero LLM cost on pages with no detectable overlay (the `blockerSignals` heuristic gates it); ~1 mini vision call (~1–3 s, ~$0.01) when a known-CMP banner *is* detected. The recon prompt's DISMISS-MODAL rule stays as a belt-and-suspenders backstop (mainly for an overlay that appears *mid*-recording, which the dismisser can't reach); the §0034 on-camera gated re-plan + graceful degradation remain the last resort. **Known limitation**: the dismisser only fires when `pageDiagnostic().blockerSignals` detects a blocker, and that heuristic is narrow (OneTrust / Cookiebot / standard `cookie-*` classes / a visible "Accept all"-type button) — Sourcepoint/TCF iframe CMPs and unusual button text slip past it. Broadening it (more hardcoded selectors) conflicts with goals.md #6 / the "no hardcoded heuristics" preference; the AI-first alternative (always run the LLM detect, no gate) costs ~$0.01 of screenshot tokens *per recording*, which is the whole goals.md #5 budget — so neither was done. Tracked as a follow-up. **Validation**: re-ran the sweep cases — Recordly `blockerDismissal: rounds 0` (no overlay), `intentSatisfaction: complete`, no regression; Guardian/CNN `rounds 0` this run too (those sites didn't surface a heuristic-matched consent banner from this IP/headless — and the Guardian failure turned out to be a wrong-but-sized target resolution, sweep finding 6, not a blocker). The dismiss *logic* is covered by unit tests (clean-page skip / one-banner / banner-then-modal / round cap / time cap / `blocker:false` / unresolvable target / LLM throws).

**Preserves**: §0034 (the prophet pipeline + rehearsing reconnoiterer), the `IReconnoiterer`/`IDirector`/`IPageSession` port signatures, `pageDiagnostic().blockerSignals` (reused as the cheap gate), the recon prompt's DISMISS-MODAL rule, `blockerDismiss=false` ⇒ exactly today's behaviour.

---

## 0036 · ref-tagged a11y snapshot target resolution (playwright-mcp's model)

**Date**: 2026-05-12

**Context**: §0034/§0021's recon emitted, per click/type step, a `target: { description }` free-text string ("the sign-in link in the header"); `LlmReconnoiterer.resolveSteps` then fed that to `stagehand.observe(description)` — a *second*, fuzzy LLM match back to a selector + bbox — and the rehearsal walk re-resolved it the same way at the right scroll position. When the page had several same-text elements, or the description was a verbose paraphrase, that second match picked the wrong element (or a 0×0 wrapper) → a dead click → divergence → reconverge re-failing the same hard target → truncated recording. This was robustness sweep #1's P2/P3 and finding 6; the interactive-first ranking + dead-click sweep + reconverge hard-check (the partial finding-6 fixes) were *recovery* patches around it, not a fix. `microsoft/playwright-mcp` solved exactly this for the agent case: it never asks the LLM to produce a re-matchable description — it shows the LLM a **deterministic ref-tagged accessibility tree** (`- button "Submit" [ref=e2]`), the LLM picks a `ref`, and `ref → locator` resolution is deterministic (`page.locator('aria-ref=e2')`). Playwright **1.59** (our installed `playwright-core`) ships this natively: `page.ariaSnapshot({ mode: 'ai', depth })` returns the ref-tagged YAML tree (incl. `<iframe>` contents) and `aria-ref=eN` resolves it. No private API.

**Options considered**: (1) just enrich `ObservedElement` with `role`/`accessibleName`/`landmark` and surface to the LLM (the earlier "increment A" — doesn't remove the fuzzy re-match, so doesn't fix the bug); (2) **ref-tagged aria snapshot + ref-based recon draft** (this); (3) full playwright-mcp parity — make recon a snapshot→act→snapshot loop (blurs the §0034 recon/playback boundary, which is the architecture we want to keep). Picked (2). *First-cut sub-decision — "pure ref" (the draft carries only a `ref`, no description) — was reverted after the first real run: on a 1448-line GitHub-repo aria tree the recon LLM picked a wrong ref (`e687` ≠ the 简中 link's `e740`), `resolveAriaRef` returned null, the click step was dropped, `intentSatisfaction` regressed `complete → unknown`. So the draft carries **`ref` + `targetDescription`** — playwright-mcp's own `target`+`element` pattern — and resolution is **ref-first, description-fallback**: this dominates pure-ref on goals #3 (intent works) and is still strictly better than the pre-§0036 status quo on #6 (the fuzzy `observe()` match is a *fallback* now, not the hot path).*

**Choice**: new `IPageSession.ariaSnapshot(opts?)` (→ `page.ariaSnapshot({ mode:'ai', depth: config.ariaSnapshotDepth })`, default depth 25; `''` on failure) + `IPageSession.resolveAriaRef(ref)` (→ `page.locator('aria-ref='+ref).boundingBox()` + derive a *durable* CSS/XPath selector via the existing `locatorSelectorFallback` + an accessible-ish name; `null` on stale/detached/0×0/no-durable-selector; never throws). New `ReconDraftSchema` / `ReconvergeDraftSchema` (`src/domain/recon-draft.ts`) — the recon LLM's output finally Zod-parsed (Hard Rule 2; previously cast via `as unknown as`); `click`/`type` carry a **`ref`** string (id into the aria tree) **plus a `targetDescription`** (the LLM's plain-English name — the fallback if the ref goes stale) instead of a `target`; scroll/key/dwell/back/done are shared verbatim with `PerformanceStepSchema` (extracted into `ScrollStepSchema`/`KeyStepSchema`/… in `performance.ts`). `LlmReconnoiterer.recon()`: `dismissBlockers → ariaSnapshot() → LLM (screenshot + tree) → ReconDraftSchema.parse → resolveDraftSteps → rehearsal walk → reset → PerformanceSchema.parse`. `resolveDraftSteps`: for each click/type, `resolveAriaRef(ref)` (deterministic, no LLM); if that misses, `resolveTargetCandidates(targetDescription)[0]` (the fuzzy fallback); if both miss, drop the step; the kept step's `target.description` is always the LLM's `targetDescription` (its intent — best fodder for the walk's dead-click sweep). Refs are resolved eagerly (valid only in the snapshot's page state). The reconverge callback takes a *fresh* `ariaSnapshot()` per call and the LLM re-picks refs (`ReconvergeDraftSchema`). The rehearsal walk **no longer re-resolves** click/type targets (it ran a per-step `observe()` LLM call — the eager aria-ref resolve is reliable, and `clickResolvedTarget` already selector-falls-back if the scrollY-0 bbox goes stale); a dead click is still caught (divergence → `sweepResolveCandidates` by `target.description` — a *recovery* path → reconverge). `UNRESOLVED_SENTINEL` / `keepUnresolvable` retired (nothing is deferred). `observeAll`/`resolveTarget`/`quickFind*` stay in the port (deprecated-by-comment, not removed — the blocker dismisser still uses `observeAll`/`resolveTarget`, and `resolveTargetCandidates` is the description-fallback + sweep). `Performance`/`PerformanceStep`/`ResolvedTarget`/`ExpectAfter` schemas, `PerformanceDirector`, `RecordJobRunner`/`RunMetrics`, the blocker dismisser, trim/judge — **all unchanged**; the on-camera `Performance` artifact is byte-for-byte the same shape, built more reliably. Recon prompts rewritten (`reconnoitererSystemPrompt` explains the tree + "give `ref` (copy the exact id) + `targetDescription` (the safety net)"; `buildReconUserText(input, snapshot)`; `buildReconvergeUserText({…, snapshot})`).

**Rationale**: serves goals.md #3 (intent — removes the most common wrong-click failure mode), #6 (AI-first / no hardcoded heuristics — the fuzzy `observe()` re-match in the hot path *was* the heuristic; the LLM now plans, deterministic code resolves; recovery is "reconverge with a fresh tree", not a fuzzy matcher), and a bit of #2 (fewer dead clicks → fewer stalls). Architecture stays swappable (#4): the aria snapshot is produced inside the Stagehand adapter, exposed as a port method returning text — same shape as `observeAll` was. **Cost trade-off (#5) — a regression, not a win**: recon's LLM *calls* drop (1× `observeAll` + N× `resolveTarget` retries → 1× recon + occasional reconverge) and recon *wall-clock* drops (~49 s → ~24 s on the canonical run), but the recon prompt's *tokens* go **up** — the aria tree (~23 k tokens for a GitHub repo page; capped at ~25 k for huge pages) replaces the old ~3 k-token observed-element list, and it's sent to the *expensive* recon model (`anthropic/claude-sonnet-4.6`, ~$3/1M) → ~$0.07–0.10 per recon vs ~$0.02–0.04 before — both already over the goals.md #5 $0.01 LLM target (which was being missed by choosing Sonnet for recon at all). To actually hit $0.01: a cheaper recon model (gpt-4o-mini at $0.15/1M → ~$0.0035 for a 23 k-token tree) or aggressive tree pruning (keep only interactive/landmark nodes — a GitHub page has ~1100 refs, ~200 genuinely actionable) or both. Tracked as a goal-#5 follow-up; the earlier draft of this ADR (and the first commit message) wrongly said "nets cheaper" — corrected here.

**Consequences**: the aria tree is the recon prompt's big input now — a GitHub repo page is ~1450 lines / ~75 KB raw, a Wikipedia featured article is **~775 KB** (`mode:'ai'` already-pruned!). A real **goal #5** pressure point, so `ariaSnapshot()` keeps it bounded: (1) `pruneAriaSnapshot` drops content/wrapper noise (`generic`/`paragraph`/`text`/`StaticText`/inline-formatting lines — keeps links/buttons/inputs/headings/landmarks/lists/tables) — ~25% off a GitHub page; (2) over `config.ariaSnapshotMaxChars` (default ~100 KB ≈ ~25 k tokens) re-snapshot scoped to `<main>`/`[role=main]`; (3) still over → truncate to the top of the tree (line-boundary cut + a "truncated — scroll for more" note). Without (2)/(3) the Cat run sent a ~180 k-token tree (~$0.50/recon). `ariaSnapshotDepth` (default 25) is the depth cap. *Even so, the typical page is ~14 k tokens on Sonnet 4.6 ≈ ~$0.04 — over the $0.01 target; pruning bought ~25%, the real lever is a cheaper recon model. Tracked.* A target that's truly absent from the snapshot (lazy-loaded / virtualized) — the LLM picks the nearest ref + a scroll step; if the resulting plan diverges, reconverge gets a fresh post-scroll tree and re-picks. The `targetDescription` fallback also covers "the LLM picked a wrong/stale ref" (which the first real run showed *will* happen on huge trees). **Validated** (ref-first, description-fallback): canonical Recordly ("click 简中, slow scroll", 10 s, headless) — `reconMs` ~24 s (down from ~49 s — the per-target `observe()` calls are gone; one `resolveTargetCandidates` fallback fired because the LLM picked a wrong ref), `totalWallClockMs` ~42 s (down from ~72 s), `rehearsal: {walkedSteps: 10, divergences: 0, …}`, `replanCount: 0`, `intentSatisfaction: complete`, judge **`LOOKS_HUMAN`** (all 5 dims pass). The regression suite — **6/6 pass, every case `intentSatisfaction: complete`** (GitHub multi-step ×2 prompt variants, YouTube search→channel ×2, Google Maps search ×2). Unit suite green (130 tests). Wikipedia "Cat → Felidae" (the original P3 hard case) re-run — **still fails** (`unknown`): the Cat article's aria tree is ~775 KB; even after the scope-to-`<main>` + truncate cap (≈100 KB) the LLM picks a bad/unresolvable ref, and the `stagehand.observe()` fallback overflows gpt-4o-mini's 128 k context. Old code would have failed there too (same `observe()` call); not a regression. The cap is a real cost win regardless (≈25 k vs ≈180 k recon tokens); a real fix for thousands-of-lines pages is a dedicated next pass — see the findings doc.

**Preserves**: §0034 (prophet pipeline + rehearsing reconnoiterer), §0035 (blocker dismisser — still uses `observeAll`/`resolveTarget`), §0030/§0031/§0033, the `IReconnoiterer`/`IDirector` port signatures and the on-camera `Performance` shape.
...
## 0038 · Transparent giant-page handling: visible-text fallback + honest `intentSatisfaction`

**Date**: 2026-05-12

**Context**: §0036's known-open case. On giant pages (Wikipedia featured articles — the "Cat" article's `mode:'ai'` aria tree is ~775 KB even after `pruneAriaSnapshot`), the recon LLM picks a wrong/unresolvable `ref` out of the truncated tree, AND the `targetDescription` → `stagehand.observe()` fallback re-serializes the whole page (~140 k tokens) over the model context → both miss → the requested click step is dropped → `RunMetrics.intentSatisfaction` reported `unknown` (the metric only went `unknown` when the Performance had zero click steps — a *dropped* click was invisible to it, so the operator saw "we don't know" when the truth was "we couldn't even attempt it"). §0036's listed options for this class: a drill-down/region-pick recon pass, a non-`observe()` fallback, or accepting `unknown` (goals.md #3 allows it).

**Options considered**: (A) drill-down recon — a coarse "which region is the target in?" LLM pass over the page's landmark/heading outline, then a region-scoped `ariaSnapshot()` and a fine `ref` pick (~1 extra LLM call on huge pages; biggest change; "recon navigates the tree"); (B) cheap deterministic fallbacks — a Playwright role/text lookup instead of `observe()` for the `ref`-miss fallback, plus surfacing dropped targets honestly (zero extra LLM cost; small; helps non-giant pages too); **(C) accept `unknown`, but try harder first AND make the failure transparent** — B's deterministic fallback, plus: never report `unknown` when a requested action was actually dropped — say `partial`/`unmet` and name what couldn't be located, annotated with "(page tree too large to analyze in full)" when the tree had to be truncated. Picked **C**. (A is tracked as the next pass if C proves insufficient — i.e. real pages where we'd genuinely want to handle a huge tree, not just report it.)

**Choice**:
- **Visible-text fallback**: `ReconDraftClickStepSchema` gains an optional `targetText` (the element's exact visible text — the LLM provides it; omitted for icon-only nodes). New port method `IPageSession.resolveByVisibleText(text)` — `getByRole('link'|'button'|'menuitem'|'tab', {name})` → `getByText(text, {exact:true})`, first sized+visible match, no LLM, **no DOM serialization** (a real locator query — scales to arbitrarily large pages where `observe()` overflows). `LlmReconnoiterer.resolveDraftSteps`' click resolution chain becomes `resolveAriaRef(ref)` → `resolveByVisibleText(targetText)` → `resolveTargetCandidates(targetDescription)[0]` → drop. Recon prompt: `click` step shape gets `"targetText"?` + a line ("the strongest fallback — a deterministic text/role lookup that works even on huge pages"). (`type` steps unchanged — textbox targets sit near the top of the tree, never truncated.) Disambiguation when several elements share `targetText` (the Cat page has multiple "Felidae" links) is a `.first()` guess, but any of them navigates to the same destination and the rehearsal walk verifies via `expectAfter.urlContains` — good enough.
- **Honest `intentSatisfaction`**: `resolveDraftSteps` now returns `{ steps, unresolved }` (the `targetDescription`s of the click/type steps it dropped); `recon()` surfaces the initial recon's `unresolved` (annotated with "(page tree too large to analyze in full)" when `snapshot.includes(ARIA_SNAPSHOT_TRUNCATION_MARKER)` — a new exported constant on the `IPageSession.ariaSnapshot` contract) as `Performance.unresolvedTargets` (optional). `RunMetrics` carries it; `computeIntentSatisfaction(hints, entries, window, unresolvedTargets = [])` factors it in: non-empty + zero planned hints → `unmet` ("couldn't locate N requested target(s): …"); non-empty + some planned hints → level capped at `partial` ("…; M other requested target(s) couldn't be located: …"). Net: `unknown` is now returned only when the Performance genuinely had no click steps AND none were dropped (the prompt asked for no click — "just scroll the page").

**Rationale**: serves goals.md #3 (intent satisfied OR *transparently* not — the metric stops saying "unknown" when it actually means "we couldn't attempt it; here's what and why") and #2 (the deterministic text lookup is cheap, no stall). #6-clean: the `targetText` lookup is AI-providing-a-signal + a generic Playwright locator query, not a hardcoded heuristic mapping; "name what we couldn't locate" is mechanical. #5 unaffected — no new LLM calls (the visible-text path is a locator query). #4: `resolveByVisibleText` is a new port method (an `IPageSession` capability — implemented in `StagehandPageSession`, faked for tests); `Performance.unresolvedTargets` is an optional schema field like `rehearsal`/`blockerDismissal`.

**Consequences**: Wikipedia "Cat → Felidae" via `npm run eval` — `intentSatisfaction` now `unmet` with note `"couldn't locate 1 requested target(s): Felidae link in the article body text (page tree too large to analyze in full); no actionable step was planned (0 click(s), 4 scroll(s), 0 type(s) still ran)"` — was bare `unknown`. The `targetText` deterministic lookup also rescues some `ref`-miss cases on ordinary pages (a link/button with literal text now resolves without an `observe()` round-trip). It does NOT make giant pages plannable — when the LLM never even sees the target in the truncated tree (or picks a bad ref and omits `targetText`), the click is still dropped, just *transparently* now. The Cat run's other failure (a scroll-only plan coming in short of `durationMs` — a §0037 duration-fidelity weakness on heavy pages, orthogonal to this) is unchanged; tracked. Drill-down recon (option A) is the next pass if "report it" isn't enough. Unit suite 144 → 151.

**Update — also surface the rehearsal walk's lost actions (2026-05-13, shipped)**: the first cut only fed the *initial* `resolveDraftSteps`' drops into `unresolvedTargets`. But `npm run eval` (post-§0039) showed `intentSatisfaction: unknown` on the GitHub language-link case: the draft's click *resolved*, the rehearsal walk *clicked* it, it diverged, the reconverge LLM came back with a click-less plan, the walk replaced the working list with that → the final `Performance` had no click step at all and `unresolvedTargets` was empty → bare `unknown`. Closed both holes: (1) the reconverge callback now accumulates its own `resolveDraftSteps` drops (a reconverged click whose ref + visible-text + `observe()` all miss) into `unresolvedTargets`; (2) backstop — if the draft asked for ≥1 click/type but the post-walk `Performance` has *none*, name those original `targetDescription`s ("(the rehearsal walk's recovery couldn't keep this in the plan)"). So that case is now `unmet` naming the link, not `unknown`. (The recon-quality root cause — the reconverge LLM ignoring its "HARD CHECK: keep the requested click" instruction on a hard page — is unchanged; drill-down recon / a stronger reconverge constraint is the eventual fix, tracked.) Unit suite 155 → 157.

**Preserves**: §0034 (prophet pipeline), §0035, §0036, §0037 (`fitPlanToBudget` still runs after `resolveDraftSteps`), §0039 (the soft-alignment is downstream), §0030/§0031/§0033, the `IReconnoiterer`/`IDirector` port signatures, the on-camera `Performance` step shapes, `RECON_REHEARSE=false` ⇒ behaviour as before.

---

## 0039 · Director duration soft-alignment (closed-loop steering to `durationMs`)

**Date**: 2026-05-13

**Context**: §0037's `fitPlanToBudget` + per-step-overhead model + video-relative trim got the trimmed-video duration from −12…+24 % down to ~+4…+12 % — but it's a *static* estimate, and a real recording's settle / per-step overhead varies (a navigation that takes 2.5 s vs an inline toggle's 0.1 s; page jank), so `npm run eval` still tipped over the ±10 % bright-line on some runs (+12 %). The estimate fundamentally can't be perfect; what can correct it is a *closed loop* — the Director knows the actual elapsed wall-clock and the target `durationMs`, so it can steer.

**Options considered**: (a) tune the `pacing*` constants more / target `durationMs × 0.92` in `fitPlanToBudget` — blunt, still open-loop; (b) **closed-loop soft-alignment in the Director** — adjust `dwell` step lengths at playback time so the recording tracks the proportional `durationMs` schedule; (c) the CDP-screencast recording layer (goals.md #4) — eliminates the clock-drift half but not the pacing-estimate half; big. Picked (b). (c) stays tracked.

**Choice**: as `PerformanceDirector.run()` plays, it tracks `renderedDeclaredMs` (the *declared* self-stated timings of steps played so far — `dwell.durationMs`, `scroll.durationMs + dwellAfterMs`, `click.anticipationMs`, `type.preMs + len·keystrokeMs`; key/back/done = 0) and `totalDeclaredMs` (same over all current `workingSteps`). Before each `dwell` step it computes `targetAfterDwell = (renderedDeclaredMs + plannedDwell) / totalDeclaredMs × durationMs` — where the recording "should" be after this dwell — and renders the dwell for `clamp(targetAfterDwell − elapsedActual, config.directorDwellMinMs, plannedDwell + config.directorDwellStretchMaxMs)` instead of `plannedDwell`. So a slow navigation that ate into the budget → the next dwell shortens (down to ~100 ms); a recording running ahead (snappy site, settles quicker than estimated) → the next dwell lengthens (capped at +2 s beyond the plan, so a tiny dwell can't balloon). `renderedDeclaredMs` always accumulates the *planned* dwell, not the adjusted one, so it stays on the declared schedule; by the last dwell `elapsedActual ≈ durationMs`. The Director's hard-budget cap (`durationMs × 1.2`) remains the ultimate backstop. Gated by a new constructor option `softAlign` (default `true`); the §0034-era director unit tests pass `softAlign: false` (the fakes' instant `waitForVisualStability` doesn't match the pacing model the alignment compares against, so it would mis-fire on them). New config: `directorDwellMinMs` (100), `directorDwellStretchMaxMs` (2000) — pure pacing parameters, goals.md #6 carve-out.

**Rationale**: serves goals.md #2 (the deliverable is the length the user paid for — now via a closed loop, not a hoped-for estimate). "Steer the recording to fill the requested duration by nudging the *pauses*" is mechanical, content-independent — goals.md #6-compatible (the bounds live in config, no hardcoded "this is too slow" threshold; the alignment doesn't touch scroll motion or click pacing — it only stretches/shrinks dwells, which read as "looking at the page" at any length in [0.1 s, planned + 2 s]). #4: `softAlign` is a constructor option; no port change.

**Consequences**: `npm run eval` on the canonical Recordly scenario — `recordingMs` lands at ~10.00 s, `endReason: done` (the closing dwell stretches/shrinks to fill exactly), trimmed-video duration `+0 %` — was ~+4…+12 % (occasionally over). Edge: if a recording runs so far ahead that even stretching every dwell to its `+2 s` cap can't reach `durationMs` (a plan with little dwell budget on a super-snappy site), it still ends short — but `fitPlanToBudget`'s pad branch (which adds a closing scroll+dwell when the recon plan is well under budget) is the off-camera complement, and a slight under-run is the safe side of the bright-line. Unit suite 151 → 155 (3 new soft-alignment tests via a `FakePageSession.waitForVisualStabilityImpl` hook + a config test). Unrelated: a separate `npm run eval` run this session had `intentSatisfaction: unknown` — the rehearsal walk's reconverge produced a click-less plan on the GitHub language-link case (recon-quality variance, the Cat-class issue from §0038's findings; the reconverge path's dropped clicks aren't yet surfaced in `unresolvedTargets` — a known transparency gap, tracked) — not a regression from this change (the soft-alignment runs after recon and can't add/remove a click step).

**Preserves**: §0034 (prophet pipeline / re-plan checkpoint / graceful degradation — the soft-alignment only changes how long a `dwell` step waits), §0035, §0036, §0037 (the `fitPlanToBudget` / `pacing*` / video-relative-trim machinery is all still in place — this is the closed-loop layer *on top* of it), §0030/§0031/§0033, the `IDirector`/`IReconnoiterer`/`IPageSession` port signatures, the on-camera `Performance` shape.
...
## 0041 · `goto` step kind — direct URL navigation as a planning primitive

**Date**: 2026-05-13

**Context**: Wild-prompts sweep ([`docs/findings/2026-05-13-wild-prompts-sweep.md`](./findings/2026-05-13-wild-prompts-sweep.md)) ran 4 shape-different prompts (open-ended nav, direct URL + interactive, front-page scan, long read) and the highest-impact gap was P1: a prompt like "打开 github 浏览下 本周热门项目 10s" starting from `https://github.com/` requires getting to `/trending`, but in 10s there is no realistic click path (header search → filter → trending tab) that fits the budget. The 7-primitive vocabulary (click/scroll/dwell/type/key/back/done) modelled "user moves their cursor"; it had no way to model "user types/pastes a URL into the address bar," which is a real human shortcut on every site for navigation users already know. Without it, A either picked a click path that overruns the budget or hallucinated a search that fails to land on /trending — both surface as `unmet`/`partial` `intentSatisfaction` even though a 1-step navigation would have satisfied the request cleanly.

**Options considered**: (A) hardcode "if prompt contains a known URL or path-like token, prepend a goto" in the runner. Heuristic, point-fix, directly violates the "no hardcoded logic" memory; can't reason about whether navigation is what the user wants vs. a search-by-URL demo. (B) extend the click vocabulary with a special "address-bar click" target. Encodes a navigation primitive as a UI-pretend click; the resolved target would be fake (no real bbox), and the Director's deterministic playback semantics fall apart. (C) **new 8th primitive `goto`** — small, symmetric with the rest of the vocabulary (a kind, an anticipationMs, a reasoning, an optional expectAfter), Zod-parsed at the boundary like every other step, planned by A using the same prompt-context the rest of the plan uses. Picked (C).

**Choice**: Add a `goto` step to `PerformanceStepSchema` and `ReconDraftStepSchema` with the shared shape `{kind:'goto', url, anticipationMs, reasoning, expectAfter?}`. A's system prompt gains a "WHEN TO USE `goto`" section ("prefer `click` when the path is in view + the budget allows; reach for `goto` when the destination URL is well-known to a user (`/trending`, a Wikipedia article slug) AND no click path fits the budget"). The schema URL must be a valid absolute URL. A **same-host hard constraint** at resolve time: the recon adapter compares `new URL(step.url).hostname` against the starting URL's hostname; mismatches are dropped and surfaced in `Performance.unresolvedTargets` with the marker `goto <url> (cross-host — same-host rule per ADR §0041)`. PerformanceDirector renders a goto as `anticipationMs wait → session.goto(url) → settle`; rehearsal walk renders it as an instant `session.goto(url)`. `sumDurations` / `fitPlanToBudget` account for goto using the same per-step cost model as click (`anticipationMs + pacingSettleEstMs`; compress scales `anticipationMs`).

**Rationale**: serves goal #6 (decision is A's — A has the prompt + start URL + budget context and decides whether goto fits; the runner enforces only a single mechanical constraint, same-host, to keep cross-origin shenanigans out); goal #3 (a cross-host goto isn't silently rewritten, it's surfaced as `unresolvedTargets` with a named reason, so `intentSatisfaction` can report `unmet`/`partial`); goal #2 (a 10s "browse trending" no longer needs to burn 6+ seconds finding the trending tab — leaves the budget for the actual browsing). The shape is symmetric with click (anticipationMs + post-step settle), so the duration accounting is straightforward and the prompt explanation is one paragraph. No new LLM calls. Same-host is the bare minimum safety: a same-host goto can land anywhere on the user's intended site (path + query are A's call); cross-host opens a class of "AI told to play with site X went to site Y" failures that have no upside.

**Consequences**: 7 → 8 action primitives. Recon prompt gains ~25 lines (WHEN TO USE goto + 1 line in the vocabulary listing + a note in DURATION & SCOPE). 4 new Zod-level tests (URL valid required, anticipationMs required, shared shape across draft + Performance) + 3 LlmReconnoiterer tests (same-host pass-through, cross-host drop into unresolvedTargets, same-host different-query passes) + 1 Director test (anticipation wait → goto → settle) + 4 prompt tests (vocabulary listed, WHEN TO USE section present, same-host constraint stated, DURATION & SCOPE includes goto). Unit test count 173 → 203. F2 (recon parsimony budget) and full F3 (judge feeds back into the metric inside the runner, not just the eval script) remain open. **Validation** deferred to a live `npm run eval` on the github.com root → "browse trending" case; baseline is the canonical Recordly run (`intentSatisfaction: complete`, `LOOKS_HUMAN`, trimmed +0%) which uses no goto and should be unaffected.

**Preserves**: §0034/§0035/§0036/§0037/§0038/§0039/§0040, the `IReconnoiterer`/`IDirector`/`IPageSession` port signatures, the on-camera `Performance` step semantics for the other 7 kinds.
