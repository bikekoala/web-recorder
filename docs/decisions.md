...
**Consequences**: the aria tree adds tokens to the recon prompt (huge content sites — bounded by `ariaSnapshotDepth`; `mode:'ai'` already prunes generic/text-only nodes); net token/wall-clock is expected to drop but **re-measure after the first real run** against the CLAUDE.md "Measured performance" table. A target that's truly absent from the snapshot (lazy-loaded / virtualized) — the LLM picks the nearest ref + a scroll step; if the resulting plan diverges, reconverge gets a fresh post-scroll tree and re-picks (the `UNRESOLVED_SENTINEL`-defer path is gone — that's the trade for the simpler flow). Validation pending: unit suite green (127 tests; new `recon-draft.test.ts`, reworked `llm-reconnoiterer`/`rehearsal`/`reconnoiterer-prompt`/`config` tests); a real `npm run prototype:stagehand` (Recordly canonical + a multi-same-text-link site like Wikipedia "Cat → Felidae" / HN "open a comment") + the regression suite still to run.
