...
**Consequences**: the aria tree is the recon prompt's big input now — a GitHub repo page is ~1450 lines / ~23 k tokens (`mode:'ai'` already prunes generic/text-only nodes; `ariaSnapshotDepth` caps depth but GitHub isn't deeper than 25 so it didn't bite there). That's a real **goal #5** pressure point — bigger than the old `observeAll` list + screenshot — so if a page blows the LLM-cost budget, scope the snapshot to a landmark or prune harder; re-measure. A target that's truly absent from the snapshot (lazy-loaded / virtualized) — the LLM picks the nearest ref + a scroll step; if the resulting plan diverges, reconverge gets a fresh post-scroll tree and re-picks. The `targetDescription` fallback also covers "the LLM picked a wrong/stale ref" (which the first real run showed *will* happen on huge trees). **Validated** (ref-first, description-fallback): canonical Recordly ("click 简中, slow scroll", 10 s, headless) — `reconMs` ~24 s (down from ~49 s — the per-target `observe()` calls are gone; one `resolveTargetCandidates` fallback fired because the LLM picked a wrong ref), `totalWallClockMs` ~42 s (down from ~72 s), `rehearsal: {walkedSteps: 10, divergences: 0, …}`, `replanCount: 0`, `intentSatisfaction: complete`, judge **`LOOKS_HUMAN`** (all 5 dims pass). The regression suite — **6/6 pass, every case `intentSatisfaction: complete`** (GitHub multi-step ×2 prompt variants, YouTube search→channel ×2, Google Maps search ×2). Unit suite green (129 tests). Wikipedia "Cat → Felidae" (the original P3 hard case) re-run — **still fails** (`unknown`): on a ~140 k-token page the LLM picks a wrong ref AND the `resolveTargetCandidates` fallback (`stagehand.observe()`) overflows gpt-4o-mini's 128 k context. Old code would have failed there too (same `observe()` call); not a regression. The lever — scope/prune the recon `ariaSnapshot()` to e.g. `<main>` (smaller tree → easier ref-picking + lower token cost) — is the next finding-6 increment, not in this change.
