...
**Rationale**: serves goals.md #3 (intent — removes the most common wrong-click failure mode), #6 (AI-first / no hardcoded heuristics — the fuzzy `observe()` re-match in the hot path *was* the heuristic; the LLM now plans, deterministic code resolves; recovery is "reconverge with a fresh tree", not a fuzzy matcher), and a bit of #2 (fewer dead clicks → fewer stalls). Nets cheaper (#5): recon's LLM calls drop from "1× `observeAll` + N× `resolveTarget` retries" (the per-target `observe()` calls were also why the walk was slow) to "1× recon (+ occasional reconverge)". Architecture stays swappable (#4): the aria snapshot is produced inside the Stagehand adapter, exposed as a port method returning text — same shape as `observeAll` was.
