 ...
 * shown. LlmReconnoiterer turns each `ref` into a {@link import('./performance.js').ResolvedTarget}
 * via `IPageSession.resolveAriaRef()` (deterministic — no second fuzzy match).
 * scroll/key/dwell/back/done are reused verbatim from `performance.ts`.
 */
